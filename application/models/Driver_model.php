...
class Driver_model extends CI_model
{
    ...
    public function signup($data_signup, $data_kendaraan, $data_berkas)
    {
        ...
        $jobselected = array(
            'id_driver' => $data_signup['id'],
            'id_job' => $data_signup['job']
        );
        
        $this->db->insert('driver_job_select', $jobselected);
    ...
    public function get_data_register($condition)
    {
        $this->db->select('driver.*, saldo.saldo,kendaraan.*');
        $this->db->from('driver');
        $this->db->join('saldo', 'driver.id = saldo.id_user');
...
public function finishori_request($cond, $condtr)
    {
    ...
    function cut_driver_saldo_by_order($data,$harga_merchant=0)
    {
        ...
        $data_fitur = $this->db->get('fitur')->row();
        // echo json_encode($data_fitur);die();
        $komisi_tipe =  $data_fitur->komisi_tipe;
        $persen = $data_fitur->komisi;

        $this->db->where('id_user', $data['id_driver']);
        $saldo = $this->db->get('saldo')->row('saldo');
        if ($data['pakai_wallet'] == 1) {
            // ongkir + total belanja
            $ongkir = $data['harga'];
            // komisi ongkir
            if($komisi_tipe == 1){
                $komisi = $ongkir * ($persen / 100);
            }else{
                $komisi = $persen;
            }
            
            // $ongkir_komisi = $ongkir - $komisi;
            
			$hasil = $harga_merchant + $ongkir;
// 			if (in_array($data['fitur'], [10,11,12,13]))
// 			$hasil = $kred + $hasil;

            $data_ins_komisi = array(
            ...
            $this->db->insert('wallet', $data_ins_komisi);
            
            $this->db->where('id_user', $data['id_driver']);
            $this->db->update('saldo', array('saldo' => ($saldo - $komisi)));
    ...
    function cut_mitra_saldo_by_order($data)
    {
        $this->db->where('id_fitur', $data['order_fitur']);
        $data_fitur = $this->db->get('fitur')->row();
        $komisi_tipe = $data_fitur->komisi_mitra_tipe;
        $persen =$data_fitur->komisi_mitra;
...
function get_driver_location($idDriver)
  ...
  function my_token($data_l)
    ...
    function jobselected($id)
    {
        $this->db->where('id_driver', $id);
        return $this->db->get('driver_job_select')->result();
    }
}
