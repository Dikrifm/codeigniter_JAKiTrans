...
class Pelanggan_model extends CI_model
{
    ...
    public function check_banned_gmail($email)
    {
 ...
 public function get_data_gmail_pelanggan($email)
    {
    ...
    public function beritahome()
    {
     ...
     public function advetorialhome()
    {
        $this->db->select('*');
        $this->db->from('advertorial');
        $this->db->join('kategori_advertorial', 'advertorial.id_kategori = kategori_advertorial.id_kategori_adv', 'left');
        $this->db->order_by('id_advertorial DESC');
        $this->db->limit('4');
        $this->db->where('status_advertorial', 1);
        return $this->db->get()->result_array();
    }
    ...
    public function beritadetail($id)
    {
     ...
     public function advetorialdetail($id)
    {
        $this->db->select('*');
        $this->db->from('advertorial');
        $this->db->join('kategori_advertorial', 'advertorial.id_kategori = kategori_advertorial.id_kategori_adv', 'left');
    ...
    public function get_driver_ridex($lat, $lng, $fitur)
    {
    ...
    public function get_driver_terdekatx($lat, $lng, $fitur,$status)
    {
        $url_foto = base_url() . 'images/fotodriver/';
     ...
     public function get_driver_ride($lat, $lng, $fitur)
    {
        ...
        $result = $this->db->query("
            ...
            WHERE f.driver_job IN (
                ...
                WHERE id_driver = d.id
            )
            AND ld.id_driver = d.id
            AND f.id_fitur = $fitur
            AND ld.status = '1'
            AND dj.id = d.job
            AND d.status = '1'
    ...
    public function get_driver_terdekat($lat, $lng, $fitur,$status)
    {
        $url_foto = base_url() . 'images/fotodriver/';
    ...
    function get_biaya()
    {
        $appsetting = $this->db->get('app_settings')->row_array();
        ...
        foreach ($biaya as $row) {
            // add time on
            $feeAddTime = 0;
            $isTimeOn = $appsetting['fee_time_status'] == 1 ? true : false;
            if($isTimeOn){
                $sekarang = strtotime(date('H:i:s'));
                $awal = strtotime($appsetting['fee_time_on']);
                $akhir = strtotime($appsetting['fee_time_off']);
                
                if($sekarang >= $awal && $sekarang <= $akhir){
                    $feeAddTime = $appsetting['fee_add_time'];
                }
            ...
            // add fee rain
            $feeRain = 0;
            $isRainOn = $appsetting['fee_rain_status'] == 1 ? true : false;
            if($isRainOn){
                $feeRain = $appsetting['fee_rain'];
            }
            
            $tempBiaya[] = array(
                'id_fitur' => $row['id_fitur'],
                'fitur' => $row['fitur'],
                'biaya' => $row['biaya'] + $feeAddTime + $feeRain,
                'icon' => $row['icon'],
                'background' => $row['background'],
                'home' => $row['home'],
                'maks_distance' => $row['maks_distance'],
                'icon_driver' => $row['icon_driver'],
                'biaya_minimum' => $row['biaya_minimum'] + $feeAddTime + $feeRain,
...
public function merchantnearby($long, $lat)
    ...
    public function merchantnearby2($long, $lat)
    {
        $this->db->select("merchant.id_merchant , merchant.nama_merchant , merchant.alamat_merchant , merchant.latitude_merchant , merchant.longitude_merchant , merchant.jam_buka , merchant.jam_tutup ,
            merchant.deskripsi_merchant , merchant.category_merchant , merchant.foto_merchant , merchant.telepon_merchant , merchant.status_merchant , merchant.open_merchant, saldo.saldo,
            (6371 * acos(cos(radians($lat)) * cos(radians(merchant.latitude_merchant)) * cos(radians(merchant.longitude_merchant) - radians( $long)) + sin(radians($lat)) * sin( radians(merchant.latitude_merchant)))) AS distance
    ...
    public function allmerchantnew($long, $lat, $fitur)
    {
        $this->db->select("merchant.id_merchant , merchant.nama_merchant , merchant.alamat_merchant , merchant.latitude_merchant , merchant.longitude_merchant , merchant.jam_buka , merchant.jam_tutup ,
            merchant.deskripsi_merchant , merchant.category_merchant , merchant.foto_merchant , merchant.telepon_merchant , merchant.status_merchant , merchant.open_merchant,saldo.saldo, mitra.created_mitra,
            (6371 * acos(cos(radians($lat)) * cos(radians(merchant.latitude_merchant)) * cos(radians(merchant.longitude_merchant) - radians( $long)) + sin(radians($lat)) * sin( radians(merchant.latitude_merchant)))) AS distance
            ,(SELECT item.status_promo FROM item where merchant.id_merchant = item.id_merchant AND status_item = 1 ORDER BY status_promo DESC LIMIT 1) AS status_promo, fitur.jarak_minimum, fitur.wallet_minimum");
        $this->db->select('category_merchant.nama_kategori');
        ...
        $this->db->where('saldo.saldo >= fitur.wallet_minimum');
        $this->db->where('DATE(mitra.created_mitra) > DATE(NOW() - INTERVAL 7 DAY)');
       // $this->db->where('saldo.saldo >= 0');
        $this->db->having("status_promo is not null");
        $this->db->having("distance <= fitur.jarak_minimum");
        
        $this->db->order_by('mitra.created_mitra DESC');
    ...
    public function allmerchantnearby2($long, $lat, $fitur)
    {
        $this->db->select("merchant.id_merchant , merchant.nama_merchant , merchant.alamat_merchant , merchant.latitude_merchant , merchant.longitude_merchant , merchant.jam_buka , merchant.jam_tutup ,
            merchant.deskripsi_merchant , merchant.category_merchant , merchant.foto_merchant , merchant.telepon_merchant , merchant.status_merchant , merchant.open_merchant,saldo.saldo,
            (6371 * acos(cos(radians($lat)) * cos(radians(merchant.latitude_merchant)) * cos(radians(merchant.longitude_merchant) - radians( $long)) + sin(radians($lat)) * sin( radians(merchant.latitude_merchant)))) AS distance
    ...
    public function allmerchantnearbybykategori2($long, $lat, $fitur, $kategori)
    {
        $this->db->select("merchant.id_merchant , merchant.nama_merchant , merchant.alamat_merchant , merchant.latitude_merchant , merchant.longitude_merchant , merchant.jam_buka , merchant.jam_tutup ,
            merchant.deskripsi_merchant , merchant.category_merchant , merchant.foto_merchant , merchant.telepon_merchant , merchant.status_merchant , merchant.open_merchant,saldo.saldo,
            (6371 * acos(cos(radians($lat)) * cos(radians(merchant.latitude_merchant)) * cos(radians(merchant.longitude_merchant) - radians( $long)) + sin(radians($lat)) * sin( radians(merchant.latitude_merchant)))) AS distance
    ...
    public function allmerchantbykategoripage($long, $lat, $fitur, $kategori, $page)
    {
        $sql = "SELECT merchant.id_merchant , merchant.nama_merchant , merchant.alamat_merchant , merchant.latitude_merchant , merchant.longitude_merchant , merchant.jam_buka , merchant.jam_tutup ,
            merchant.deskripsi_merchant , merchant.category_merchant , merchant.foto_merchant , merchant.telepon_merchant , merchant.status_merchant , merchant.open_merchant,saldo.saldo,merchant.id_fitur,fitur.home,
            (6371 * acos(cos(radians($lat)) * cos(radians(merchant.latitude_merchant)) * cos(radians(merchant.longitude_merchant) - radians( $long)) + sin(radians($lat)) * sin( radians(merchant.latitude_merchant)))) AS distance
            ,(SELECT item.status_promo FROM item where merchant.id_merchant = item.id_merchant AND status_item = 1 ORDER BY status_promo DESC LIMIT 1) AS status_promo, fitur.jarak_minimum, fitur.maks_distance, fitur.wallet_minimum, category_merchant.nama_kategori

        FROM `merchant`
        LEFT JOIN `fitur` ON `merchant`.`id_fitur` = `fitur`.`id_fitur`
        LEFT JOIN `mitra` ON `mitra`.`id_merchant` = `merchant`.`id_merchant`
        LEFT JOIN `saldo` ON `mitra`.`id_mitra`    = `saldo`.`id_user`
        LEFT JOIN `category_merchant` ON `merchant`.`category_merchant` = `category_merchant`.`id_kategori_merchant`
        WHERE `merchant`.`status_merchant` = 1 
        AND `mitra`.`status_mitra` = 1";

        if($fitur > 0){
            $sql .= " AND merchant.id_fitur = $fitur ";
        }


        $sql .= " AND saldo.saldo >= fitur.wallet_minimum ";
        if($kategori != '1'){
            $sql .= " AND merchant.category_merchant = $kategori ";
           
        }
        
        $sql .= "HAVING status_promo is not null";
        $sql .= " AND distance <= fitur.jarak_minimum ORDER BY distance ASC ";
    ...
    public function merchantpromobytrx($fitur, $long, $lat)
    {
	
		$sql="SELECT merchant.*,`fitur`.`jarak_minimum`, `fitur`.`maks_distance`, `fitur`.`wallet_minimum`,
		(SELECT item.status_promo FROM item where merchant.id_merchant = item.id_merchant AND status_item = 1 ORDER BY status_promo DESC LIMIT 1) AS status_promo,
        	(6371 * ACOS(COS(RADIANS($lat)) * COS(RADIANS(merchant.latitude_merchant)) * COS(RADIANS(merchant.longitude_merchant) - RADIANS($long)) + SIN(RADIANS($lat)) * SIN( RADIANS(merchant.latitude_merchant)))) AS distance
        FROM
        	(SELECT merchant.*, mitra.status_mitra, mitra.saldo, '1' AS `status_promo` FROM
        		(SELECT `merchant`.`id_merchant`, `merchant`.`nama_merchant`, `merchant`.`alamat_merchant`, `merchant`.`latitude_merchant`, `merchant`.`longitude_merchant`, 
        			`merchant`.`jam_buka`, `merchant`.`jam_tutup`, `merchant`.`deskripsi_merchant`, `merchant`.`category_merchant`, `merchant`.`foto_merchant`,
        			`merchant`.`telepon_merchant`, `merchant`.`status_merchant`, `merchant`.`open_merchant`, a.id_trx, merchant.`id_fitur`
        		FROM
        			(SELECT MAX(id_trans_merchant) AS id_trx, id_merchant FROM transaksi_detail_merchant GROUP BY id_merchant) a
        		LEFT JOIN merchant 
        		ON a.id_merchant = merchant.id_merchant
        		) merchant
        	LEFT JOIN
        		(SELECT mitra.id_mitra, mitra.status_mitra, mitra.id_merchant, saldo.saldo
        		FROM mitra
        		LEFT JOIN saldo
        		ON mitra.id_mitra = saldo.id_user) mitra	
        	ON merchant.id_merchant = mitra.`id_merchant`
        	WHERE merchant.id_merchant IN (SELECT DISTINCT(id_merchant) FROM item )
        	) merchant
        LEFT JOIN fitur
        ON `merchant`.`id_fitur` = `fitur`.`id_fitur`
        WHERE merchant.`status_merchant`=1
        AND merchant.id_fitur = $fitur
        HAVING distance <= fitur.`maks_distance`
    ...
    public function searchmerchantnearby2($like, $long, $lat, $fitur)
    {
        $this->db->select("merchant.id_merchant ,
         merchant.nama_merchant , 
         merchant.alamat_merchant , 
        ...
        $this->db->where("saldo.saldo >= fitur.wallet_minimum AND merchant.id_fitur = $fitur");
       // $this->db->where("saldo.saldo >= 0 AND merchant.id_fitur = $fitur");
        $this->db->where('mitra.status_mitra = 1');
        $this->db->where('merchant.status_merchant = 1');
    ...
    public function merchantbyid2($idmerchant, $long, $lat)
    {
        ...
        $this->db->select('category_merchant.nama_kategori');
        //$this->db->select('fitur.fitur, fitur.jarak_minimum, fitur.wallet_minimum');
        $this->db->select('fitur.fitur, fitur.jarak_minimum');
        $this->db->select('saldo.saldo');
        $this->db->where("merchant.id_merchant = $idmerchant");
        $this->db->where('merchant.status_merchant = 1');
        $this->db->where('saldo.saldo >= fitur.wallet_minimum');
    ...
    function mylokasi($data_l)
    {
        ...
        $this->db->where('id_user', $data_l['id_user']);
        $upd = $this->db->update('config_user', $data);
    ...
    function statusku($data_l)
    {
        if ($data_l['israte'] != '0') {
            $data = array(
                'israte' => $data_l['israte'],
                'id_driver' => $data_l['id_driver'],
                'id_transaksi' => $data_l['id_transaksi'],
                'total_biaya' => $data_l['total_biaya'],
                'pakai_wallet' => $data_l['pakai_wallet'],
                'fitur' => $data_l['fitur'],
                'nama_driver' => $data_l['nama_driver'],
                'foto_driver' => $data_l['foto_driver'],
                'point_driver' => $data_l['point_driver'],
                'response' => $data_l['response']
            );
        } else {
            $data = array(
                'israte' => $data_l['israte'],
        ...
        $upd = $this->db->update('pelanggan', $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    public function get_status($idtrans)
    {
        $this->db->select('*');
        $this->db->from('history_transaksi');
    ...
    function save_home($data_lokasi)
    {
        $this->db->select('*');
        $this->db->from('lokasi_pelanggan');
        $this->db->where('id_pelanggan', $data_lokasi['id_pelanggan']);
        $this->db->limit('1');
        $id = $this->db->get();
        if ($id->row('nama') == 'Rumah') {
                 $this->db->where('id_pelanggan', $data_lokasi['id_pelanggan']);
                 $this->db->update('lokasi_pelanggan', $data_lokasi);
                 return true;
            } else if($id->row('utama') == '0') {
                return false;
            } else {
    ...
    function save_lokasi($data_lokasi)
    {
        $this->db->select('*');
        $this->db->from('lokasi_pelanggan');
        $this->db->where('id_pelanggan', $data_lokasi['id_pelanggan']);
    ...
    function get_list_lokasi($iduser)
    {
       $this->db->select('*');
        $this->db->from('lokasi_pelanggan');
        $this->db->where('lokasi_pelanggan.id_pelanggan', $iduser);
        $this->db->where('lokasi_pelanggan.nama','Rumah');
        $this->db->limit(1);
        $this->db->order_by('lokasi_pelanggan.id', 'DESC');
        $trans = $this->db->get();
        return $trans;
    }
    
    function list_lokasi_pelanggan($iduser)
    {
       $this->db->select('*');
        $this->db->from('lokasi_pelanggan');
        $this->db->where('lokasi_pelanggan.id_pelanggan', $iduser);
    ...
    public function sliderapp($idfitur)
    {
        $this->db->select('*');
        $this->db->from('promosi');
        $this->db->where('promosi.fitur_promosi', $idfitur);
        $this->db->where('is_show', '1');
        $this->db->where('tanggal_berakhir>', date("Y-m-d"));
        return $this->db->get()->result_array();
    }
    ...
    function ceklogin($iduser)
    {
       $this->db->select('*');
        $this->db->from('pelanggan');
        $this->db->where('pelanggan.id', $iduser);
        $trans = $this->db->get();
        return $trans;
    }
    ...
    function all_inbox($iduser)
    {
        $this->db->select('*');
       $this->db->where('inbok.id', $iduser);
        $this->db->from('inbok');
        $this->db->order_by('inbok.id', 'DESC');
        $trans = $this->db->get();
        return $trans;
    }
    ...
    function data_fiturpromo($fitur)
    {
        $this->db->select('*');
        $this->db->where('kodepromo.fitur', $fitur);
        $this->db->from('kodepromo');
        $this->db->where('status', '1');
        $trans = $this->db->get();
        return $trans;
    ...
    public function check_homelok($nama)
    {
        $cek = $this->db->query("SELECT id_pelanggan FROM lokasi_pelanggan where nama = '$nama'");
        if ($cek->num_rows() == 1) {
            return true;
        } else {
            return false;
    ...
    //-------------------------- PPOB ---------------------------------
     public function list_ppob($tipe)
    {
        if($tipe == NULL || $tipe == ''){
            $this->db->select('*');
            $this->db->from('ppob_fitur');
            $this->db->where('status', '1');
        }else{
            $this->db->select('*');
            $this->db->from('ppob_fitur');
    ...
    function historippob($data_histori)
    {
        $this->db->select('*');
        $this->db->from('ppob_histori');
        $this->db->where('iduser', $data_histori['iduser']);
        $this->db->limit('1');
        $this->db->insert('ppob_histori', $data_histori);
        return true;
        
    }
    function data_ppobhistori($id)
    {
        $this->db->select('*');
        $this->db->where('iduser', $id);
        $this->db->from('ppob_histori');
        $this->db->order_by('trx DESC');
        $trans = $this->db->get();
        return $trans;
    }
    function datacek_ppobhistori($id)
    {
        $this->db->select('*');
        $this->db->where('iduser', $id);
    ...
    function updateppobhistori($data_l)
    {
        $data = array(
                'status' => $data_l['status']
        );
        $dataC = array(
                'id_pelanggan' => $data_l['iduser'],
                'nama_pelanggan' => $data_l['nama'],
                'mysaldo' => $data_l['mysaldo'],
                'harga' => $data_l['harga']
        );
        $this->db->select('trx');
        $this->db->from('ppob_histori');
        $this->db->where('reff', $data_l['reff']);
        $this->db->update('ppob_histori', $data);
    ...
    {   
        ...
        $this->db->group_by('transaksi_saldo.regtime');
