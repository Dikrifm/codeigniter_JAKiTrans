...
class Driver extends CI_Controller
{
    ...
    public function detail($id)
    {
        ...
        $jobselected = $this->driver->jobselected($id);
        
        $menu_checked = array();
        foreach ($jobselected as $value) {
            array_push($menu_checked, $value->id_job);
        }
        
        $data['jobselected'] = $menu_checked;
            ...
            //    $this->session->set_flashdata('demo', 'NOT ALLOWED FOR DEMO');
             //   redirect('driver/detail/' . $this->input->post('id', TRUE));
              $id = html_escape($this->input->post('id', TRUE));
                ...
                // input selected job
                $jobselected = $this->input->post('job', TRUE);
                if(count($jobselected) > 0){
                    $this->db->where('id_driver', $id);
                    $this->db->delete('driver_job_select');
                    ...
                    $this->db->insert_batch('driver_job_select', $datajobs);
                    
                }
                
                
                // echo json_encode($jobselected);die();
