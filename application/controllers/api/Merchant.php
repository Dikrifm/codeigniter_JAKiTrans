...
class Merchant extends REST_Controller
{
    ...
    function login_post()
    {
        ...
        if ($check_banned) {
            ...
            if ($cek_login->num_rows() > 0) {
                $row_user = $cek_login->row();
                if($row_user->status_mitra > 0){
                    ...
                    $message = array(
                        'code' => '201',
                        'message' => 'Akun kamu masih menunggu konfirmasi, Silahkan hubungi CS Admin JAKiTrans',
                    );
                    $this->response($message, 200);
                }
            ...
            if ($dec_data->checked == "false"){
                ...
                if ($signup) {
                    $message = array(
                        'code' => '200',
                        'message' => 'success',
                        'data' => 'Pendaftaran Berhasil, Silahkan hubungi CS Admin JAKiTrans, Mohon Tunggu Untuk konfirmasi!',
                        'user' => $data_signup
                    );
                    $this->response($message, 200);
                } else {
    ...
    function add_kategoris_post()
    {
        ...
        $cek_login = $this->Merchantapi_model->get_data_merchant($condition);
        $this->Merchantapi_model->tambahkategori($dataitem);
        if ($cek_login->num_rows() > 0) {
            $message = array(
                'code' => '200',
                'message' => 'success'
    ...
    function add_kategori_post()
    {
        ...
        $cek_login = $this->Merchantapi_model->get_data_merchant($condition);
        $this->Merchantapi_model->addkategori($dec_data->namakategori, $dec_data->status, $dec_data->id,$dec_data->foto);
        if ($cek_login->num_rows() > 0) {
            $message = array(
                'code' => '200',
                'message' => 'success'
